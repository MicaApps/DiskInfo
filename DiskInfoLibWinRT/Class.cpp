...
namespace winrt::DiskInfoLibWinRT::implementation
{
	...
	void Class::SaveText(winrt::hstring fileName)
	{
		auto const& m_Ata = CAtaSmart::get_instance();
...
");
		...
		else
		{
			...
			CFile file;
			if (file.Open(fileName.data(), CFile::modeCreate | CFile::modeWrite))
			{
	...
	CHAR Class::AsciiFilter(BYTE ch)
	{
	...
	CString Class::GetDiskStatus(DWORD statusCode)
	{
		switch (statusCode)
		{
